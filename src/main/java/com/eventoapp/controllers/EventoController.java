...
public class EventoController {
	...
	public ModelAndView formEditarEvento(long codigo) {
		Evento evento = er.findByCodigo(codigo);
		ModelAndView mv = new ModelAndView("evento/detalhesEventoEditar");
		mv.addObject("evento", evento);
		return mv;
	}
	...
	public String updateEvent(@Valid Evento evento, BindingResult result, RedirectAttributes attributes) {
		attributes.addAttribute("codigo", evento.getCodigo());
		if (result.hasErrors()) {
			attributes.addFlashAttribute("mensagem", "Verifique os campos!");
	...
	public String salvaConvidado(@PathVariable("codigo") long codigo, @Valid Convidado convidado, BindingResult result,
